...
//post: finds the most fit and repopulates forest with mutations of the most fit 
   private static void advanceGeneration()
   {
      ...
      {
         if(forest[j].compareTo(forest[0]) == 1)
            forest[0] = forest[j];
      }
      for(int i = 1; i < forest.length; i++)
      {
   ...
   */
      public void keyPressed(KeyEvent e)
      {
         ...
         //***COMPLETE THE CODE HERE*****************************************
            Tree[] temp = forest.clone();
            forest = new Tree[temp.length-1];
         ...
         Tree[] temp = forest.clone();
            ...
            for(int i = 0; i < temp.length; i++)
               forest[i] = temp[i];
               int x = forest.length-1;
               forest[forest.length-1] = forest[0].mutate(MUTATION_RATE);
         //******************************************************************/
            repaint();
            return;
         }
